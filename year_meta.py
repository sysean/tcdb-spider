import json
from http_utils import send
from log import setup_logging
from save_db import insert_dataset, insert_year_dataset_output_batch

setup_logging()

category = "Baseball"

years = [
    1866, 1868, 1869, 1870, 1871, 1872, 1874, 1875, 1876, 1877, 1878, 1879,
    1880, 1881, 1882, 1885, 1886, 1887, 1888, 1889, 1890, 1891, 1892, 1893,
    1894, 1895, 1896, 1897, 1899, 1900, 1901, 1902, 1903, 1904, 1905, 1906,
    1907, 1908, 1909, 1910, 1911, 1912, 1913, 1914, 1915, 1916, 1917, 1918,
    1919, 1920, 1921, 1922, 1923, 1924, 1925, 1926, 1927, 1928, 1929, 1930,
    1931, 1932, 1933, 1934, 1935, 1936, 1937, 1938, 1939, 1940, 1941, 1942,
    1943, 1944, 1945, 1946, 1947, 1948, 1949, 1950, 1951, 1952, 1953, 1954,
    1955, 1956, 1957, 1958, 1959, 1960, 1961, 1962, 1963, 1964, 1965, 1966,
    1967, 1968, 1969, 1970, 1971, 1972, 1973, 1974, 1975, 1976, 1977, 1978,
    1979, 1980, 1981, 1982, 1983, 1984, 1985, 1986, 1987, 1988, 1989, 1990,
    1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002,
    2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014,
    2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024
]


# 测试获取某个 year 下所有的集合列表
def get_set_list_by_year(year: int):
    url = f"https://www.tcdb.com/ViewAll.cfm/sp/{category}/year/{year}"

    soup = send(url, msg="获取某个 year 下所有的集合列表")

    block1_divs = soup.find_all('div', class_='block1')
    index = 0

    li_list = []
    for block1 in block1_divs:
        first_child = block1.find()
        if first_child and first_child.name == 'a':
            ul_tags = block1.find_all('ul')

            for ul in ul_tags:
                for li in ul.find_all('li'):
                    a_tag = li.find('a')
                    if a_tag:
                        li_list.append({
                            "name": a_tag.get_text(strip=True),
                            "sid": a_tag['href'].split('/')[-2]
                        })
                index += 1

    batch = []
    for set_msg in li_list:
        batch.append({
            "category": category,
            "year": year,
            "sid": set_msg["sid"],
            "set_name": set_msg["name"]
        })

    insert_year_dataset_output_batch(batch)


# 运行打开此注释
for year in years:
    get_set_list_by_year(year)
